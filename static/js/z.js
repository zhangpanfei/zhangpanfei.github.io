...
function mypjax () {
  ...
  if (!(url.indexOf('/music') > -1)) {
    for (var i in aplist) {
      aplist[i].destroy && aplist[i].destroy()
    }
    aplist = []
  } else {
    $.ajax({
      url: 'https://api.fczbl.vip/163/?type=playlist&id=4869147611',
...
function photo () {
  ...
  ;(function () {
    ...
    return {
      // --- load data ----
      load: function (data) {
        ge1doot.loadJS('/wp-content/themes/kratos-pjax/static/js/imageTransform3D.js', init, data)
      }
    }
  })().load({
    imgdata: [
      ...
      {
        img:
          '/wp-content/themes/kratos-pjax/static/images/thumb/thumb_1.jpg',
        x: -1000,
        y: 0,
        z: 1500,
        nx: 0,
